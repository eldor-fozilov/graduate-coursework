...
for task_idx, (x_train_task, t_train_task, x_test_task, t_test_task) in enumerate(dataset.task_data):
    ...
    if task_idx == 0:
        random_samples = random.sample(range(len(x_train_task)), num_of_samples) # sample 500 samples from the first task
        all_x_train_task.append(x_train_task[random_samples])
        all_t_train_task.append(t_train_task[random_samples])
    else:
