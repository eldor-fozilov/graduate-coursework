...
def briefMatch(desc1, desc2, ratio=0.8):
    ...
    matches = skimage.feature.match_descriptors(
        desc1, desc2, 'hamming', cross_check=True, max_ratio=ratio)
    return matches


def saveMatches(im1, im2, matches, locs1, locs2, filename):
    fig, ax = plt.subplots(nrows=1, ncols=1)
    im1 = cv2.cvtColor(im1, cv2.COLOR_BGR2GRAY)
    im2 = cv2.cvtColor(im2, cv2.COLOR_BGR2GRAY)
    plt.axis('off')
    skimage.feature.plot_matches(
        ax, im1, im2, locs1, locs2, matches, matches_color='b', only_matches=True)
...
def computeBrief(img, locs):
    ...
    desc = np.array([list(map(lambda x: computePixel(
        img, x[0], x[1], patchWidth, c), zip(compareX, compareY))) for c in locs])

    return desc, locs


def corner_detection(im, n=12, thres=0.15):
    # fast method
    result_img = skimage.feature.corner_fast(im, n, thres)
